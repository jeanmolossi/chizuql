...
- As configurações de lint devem usar apenas chaves previstas no schema oficial: para `wsl_v5`, utilize os campos `allow-first-in-block`, `allow-whole-block`, `branch-max-lines`, `case-max-lines`, `default`, `enable` e `disable`; para `issues` na raiz, limite-se a `max-issues-per-linter`, `max-same-issues`, `new`, `new-from-merge-base`, `new-from-rev`, `new-from-patch`, `fix`, `uniq-by-line` e `whole-files`.
...
- As pipelines de CI devem manter, no mínimo, as versões atuais das actions utilizadas: `actions/checkout@v6.0.0`, `actions/setup-go@v6.1.0`, `actions/cache@v4.3.0`, `golangci/golangci-lint-action@v9.1.0`, `github/codeql-action/*@v4.31.4`, `actions/upload-artifact@v5.0.0`, `actions/download-artifact@v6.0.0` e `actions/dependency-review-action@v4.8.2`. Não faça downgrade; ao atualizar qualquer versão, ajuste esta instrução para que o novo número passe a ser o mínimo aceito.
